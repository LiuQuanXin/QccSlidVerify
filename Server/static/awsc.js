! function(t, a) {
    ...
    function E(t, r, s) {
        if (s)
            for (var c = 0; c < t.length; c++) {
                var v = t[c];
                v = x(v), a.write("<script src=" + v + "></script>")
            } else
                for (var c = 0; c < t.length; c++) {
                    // var v = t[c];
                    if ( t[c].indexOf('118') != -1)
                        var v = '//127.0.0.1:8880/static/1181.js';
                    else if (t[c].indexOf('um')!= -1)
                        var v = '//127.0.0.1:8880/static/um.js';
                    v = x(v);
    ...
    function D(t, a, r) {
        ...
        if (0 !== s.depends.length)
            for (var w = 0; w < s.depends.length; w++) {
                var M = s.depends[w];
                r && (delete r.sync, delete r.timeout, delete r.cdn), U(M, void 0, r)
            }
        var k = {};
        if (k.module = s, k.name = t, k.state = p, k.callbacks = [], k.options = r, a && k.callbacks.push(a), k.timeoutTimer = setTimeout(function() {
            k.state = b, T(k, r && r.throwExceptionInCallback)
    ...
    function U(t, a, r) {
        var s = W(t);
        return s ? (s.state === h || s.state === b ? a && a(s.state, s.exportObj) : s.state === p ? a && s.callbacks.push(a) : void 0, void 0) : (D(t, a, r), void 0)
    }
    ...
    function O(t, a, r) {
        ...
        try {
            ...
            for (var y = void 0, j = 0; j < v.length; j++) {
                var S = v[j];
                S.module === c && S.name === a && (s = S.options && S.options.throwExceptionInCallback, y = S, clearTimeout(y.timeoutTimer), delete y.timeoutTimer, y.exportObj = r, S.state === p || S.state === b ? (y.state = h, T(y, s)) : void 0)
            }
            y || (y = {}, y.module = c, y.name = a, y.state = h, y.exportObj = r, y.callbacks = [], v.push(y))
        } catch (e) {
            if (s) throw e;
    ...
    function R() {
        t.AWSC || (t.AWSC = {}, t.AWSC.use = U, t.AWSCInner = {}, t.AWSCInner.register = O)
    }
    R()
}(window, document);